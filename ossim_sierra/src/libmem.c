...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  /* Sử dụng syscall để mở rộng giới hạn bộ nhớ */
  struct sc_regs regs;
  regs.a1 = SYSMEM_INC_OP; // Loại thao tác: mở rộng giới hạn
  regs.a2 = vmaid;         
  regs.a3 = inc_sz;        

  if (syscall(caller, 17, &regs) != 0)
  {
    /* Nếu syscall thất bại, khôi phục giá trị cũ */
  ...
  if (inc_sz > size) {
    struct vm_rg_struct* fNode = malloc(sizeof(struct vm_rg_struct));
    fNode->rg_start = old_sbrk + size;
    fNode->rg_end = old_sbrk + inc_sz;
    
    enlist_vm_freerg_list(caller->mm, fNode);
  }
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    regs.a1 = SYSMEM_SWP_OP;
    regs.a2 = PAGING_PTE_SWP(pte);
    regs.a3 = vicfpn;
    syscall(caller, 17, &regs);

    // __swap_cp_page(caller->active_mswp, swpfpn, caller->mram, pgn);
...
int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller)
{
  ...
  /* Sử dụng syscall để ghi giá trị */
  struct sc_regs regs;
